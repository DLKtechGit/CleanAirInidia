...
useEffect(() => {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.1.77:5000/api/blogs?${queryParams.toString()}`, {
        // headers: {
        //   'Authorization': `Bearer ${token}`
        // }
      });
  ...
  const fetchSearchSuggestions = async (searchTerm) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://192.168.1.77:5000/api/blogs?search=${encodeURIComponent(searchTerm)}&limit=5`, {
        // headers: {
        //   'Authorization': `Bearer ${token}`
        // }
      });
