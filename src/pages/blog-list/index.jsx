...
const BlogList = () => {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const response = await fetch(`http://46.202.166.103:5000/api/blogs?${params}`, {
  ...
  const fetchAllBlogsForStats = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://46.202.166.103:5000/api/blogs?limit=1000`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDelete = async (blogId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://46.202.166.103:5000/api/blogs/${blogId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleBulkDelete = async (blogIds) => {
    try {
      ...
      const deletePromises = blogIds.map(id => 
        fetch(`http://46.202.166.103:5000/api/blogs/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
